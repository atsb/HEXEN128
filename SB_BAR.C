...
#if (APPVER_HEXENREV >= AV_HR_HEX10A) && (defined __WATCOMC__)
//#ifdef __WATCOMC__
...
#if (APPVER_HEXENREV < AV_HR_HEX10A) // VERSIONS RESETORATION: Helper macro
#define APPVER_ULTMSG_ARG false
#else
#define APPVER_ULTMSG_ARG true
#endif
...
static void CheatFinaleFunc(player_t *player, Cheat_t *cheat);
#endif
...
#endif
#elif (APPVER_HEXENREV < AV_HR_HEX10A) // && (!defined APPVER_DEMO)
// Toggle god mode
static byte CheatGodSeq[] =
{
	CHEAT_ENCRYPT('m'),
...
#else // (APPVER_HEXENREV >= AV_HR_HEX10A) && (!defined APPVER_DEMO)
// Toggle god mode
static byte CheatGodSeq[] =
{
	CHEAT_ENCRYPT('s'),
...
#endif // APPVER_HEXENREV / APPVER_DEMO
...
#if (APPVER_HEXENREV < AV_HR_HEX10A)
	{ CheatFinaleFunc, CheatFinaleSeq, NULL, 0, 0, 0 },
#endif
...
static void CheatGodFunc(player_t *player, Cheat_t *cheat)
{
	...
	{
		P_SetMessage(player, TXT_CHEATGODON, APPVER_ULTMSG_ARG);
	}
	else
	{
		P_SetMessage(player, TXT_CHEATGODOFF, APPVER_ULTMSG_ARG);
	}
	SB_state = -1;
}
...
static void CheatNoClipFunc(player_t *player, Cheat_t *cheat)
{
	...
	{
		P_SetMessage(player, TXT_CHEATNOCLIPON, APPVER_ULTMSG_ARG);
	}
	else
	{
		P_SetMessage(player, TXT_CHEATNOCLIPOFF, APPVER_ULTMSG_ARG);
	}
}
...
static void CheatWeaponsFunc(player_t *player, Cheat_t *cheat)
{
	...
	P_SetMessage(player, TXT_CHEATWEAPONS, APPVER_ULTMSG_ARG);
}
...
static void CheatHealthFunc(player_t *player, Cheat_t *cheat)
{
	...
	P_SetMessage(player, TXT_CHEATHEALTH, APPVER_ULTMSG_ARG);
}

static void CheatKeysFunc(player_t *player, Cheat_t *cheat)
{
	player->keys = 2047;
	P_SetMessage(player, TXT_CHEATKEYS, APPVER_ULTMSG_ARG);
}
...
static void CheatSoundFunc(player_t *player, Cheat_t *cheat)
{
	...
	if(DebugSound)
	{
		P_SetMessage(player, TXT_CHEATSOUNDON, APPVER_ULTMSG_ARG);
	}
	else
	{
		P_SetMessage(player, TXT_CHEATSOUNDOFF, APPVER_ULTMSG_ARG);
	}
}
...
static void CheatTickerFunc(player_t *player, Cheat_t *cheat)
{
	...
	if(DisplayTicker)
	{
		P_SetMessage(player, TXT_CHEATTICKERON, APPVER_ULTMSG_ARG);
	}
	else
	{
		P_SetMessage(player, TXT_CHEATTICKEROFF, APPVER_ULTMSG_ARG);
	}
}
...
static void CheatArtifactAllFunc(player_t *player, Cheat_t *cheat)
{
	...
	P_SetMessage(player, TXT_CHEATARTIFACTS3, APPVER_ULTMSG_ARG);
}
...
static void CheatInitFunc(player_t *player, Cheat_t *cheat)
{
	...
	P_SetMessage(player, TXT_CHEATWARP, APPVER_ULTMSG_ARG);
}
...
static void CheatWarpFunc(player_t *player, Cheat_t *cheat)
{
	...
	{ // Bad map
		P_SetMessage(player, TXT_CHEATBADINPUT, APPVER_ULTMSG_ARG);
		return;
	}
#if (APPVER_HEXENREV < AV_HR_HEX10A)
	...
	if(map == -1)
	{ // Not found
		P_SetMessage(player, TXT_CHEATNOMAP, APPVER_ULTMSG_ARG);
		return;
	}
	if(map == gamemap)
...
static void CheatPigFunc(player_t *player, Cheat_t *cheat)
{
	...
	P_SetMessage(player, "SQUEAL!!", APPVER_ULTMSG_ARG);
}
...
#if (APPVER_HEXENREV < AV_HR_HEX10A)
	P_Massacre();
	P_SetMessage(player, "CARNAGE", false);
#else
	int count;
	char buffer[80];
...
static void CheatClassFunc1(player_t *player, Cheat_t *cheat)
{	
	P_SetMessage(player, "ENTER NEW PLAYER CLASS (0 - 2)", APPVER_ULTMSG_ARG);
}
...
static void CheatClassFunc2(player_t *player, Cheat_t *cheat)
{
	...
	if(class > 2 || class < 0)
	{
		P_SetMessage(player, "INVALID PLAYER CLASS", APPVER_ULTMSG_ARG);
		return;
	}
	player->class = class;
	for(i = 0; i < NUMARMOR; i++)
...
static void CheatVersionFunc(player_t *player, Cheat_t *cheat)
{
	P_SetMessage(player, VERSIONTEXT, APPVER_ULTMSG_ARG);
}
...
static void CheatDebugFunc(player_t *player, Cheat_t *cheat)
{
	...
	sprintf(textBuffer, "MAP %2d  X:%5d  Y:%5d  Z:%5d",
				gamemap,
	...
	sprintf(textBuffer, "MAP %d (%d)  X:%5d  Y:%5d  Z:%5d",
				...
				player->mo->z >> FRACBITS);
#endif
	P_SetMessage(player, textBuffer, APPVER_ULTMSG_ARG);
}

static void CheatScriptFunc1(player_t *player, Cheat_t *cheat)
{
	P_SetMessage(player, "RUN WHICH SCRIPT(01-99)?", APPVER_ULTMSG_ARG);
}
...
static void CheatScriptFunc3(player_t *player, Cheat_t *cheat)
{
	...
	{
#if (APPVER_HEXENREV < AV_HR_HEX10A)
		sprintf(textBuffer, "RUNNING SCRIPT %d", script);
#else
