...
#if (APPVER_HEXENREV < AV_HR_HEX10A)
void A_PlayerBurn(mobj_t *actor)
{
}
#endif
...
#if (APPVER_HEXENREV < AV_HR_HEX10A) // Partially reconstructed using P_CreateTIDList
void A_FlyCheck(mobj_t *actor)
{
	thinker_t *t;
...
void A_FlyMove(mobj_t *actor)
{
	int angle;
	int r, x, y, z;
	mobj_t *target = actor->target;

	if (!target || !(target->flags & MF_CORPSE) || (P_Random() < 5))
	{
	...
	if (actor->args[0] & 2)
	{
		actor->momx += (P_Random() - 128) << 10;
		actor->momy += (P_Random() - 128) << 10;
	}

	r = P_Random();
	z = r << 10;
	if (target->z + 5*FRACUNIT < actor->z)
	{
		if (r > 150)
		{
			actor->momz = -z;
...
#if (APPVER_HEXENREV < AV_HR_HEX10A)
			P_GivePower(((mobj_t *)actor->special1)->player, pw_minotaur);
#else
...
void A_Quake(mobj_t *actor)
{
	...
	if (actor->args[1]-- > 0)
	{
		for (playnum=0; playnum < MAXPLAYERS; playnum++)
		{
			...
			victim = player->mo;
#if (APPVER_HEXENREV < AV_HR_HEX10A)
			if (playnum != consoleplayer) continue;
#endif
			...
			{
#if (APPVER_HEXENREV < AV_HR_HEX10A)
				victim->flags2 |= MF2_DONTDRAW;
				localQuakeHappening = richters;
#else
	...
	else
	{
		for (playnum=0; playnum < MAXPLAYERS; playnum++)
		{
			...
			if (playeringame[playnum] &&
			    player && playnum == consoleplayer)
				localQuakeHappening = false;
#else
...
void A_SoAExplode(mobj_t *actor)
{
	...
	if(actor->args[0])
	{ // Spawn an item
#if (APPVER_HEXENREV >= AV_HR_HEX11B)
		if(!nomonsters 
		|| !(mobjinfo[TranslateThingType[actor->args[0]]].flags&MF_COUNTKILL))
#endif
		{ // Only spawn monsters if not -nomonsters
