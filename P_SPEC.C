...
static boolean CheckedLockedDoor(mobj_t *mo, byte lock)
{
	...
	{
#if (APPVER_HEXENREV < AV_HR_HEX10A)
		strcpy(LockedBuffer, "YOU NEED THE ");
		strcat(LockedBuffer, TextKeyMessages[lock-1]);
		strcat(LockedBuffer, "\n");
...
boolean EV_LineSearchForPuzzleItem(line_t *line, byte *args, mobj_t *mo)
{
	...
	for (i=0; i<player->artifactCount; i++)
	{
		...
		{
#if (APPVER_HEXENREV < AV_HR_HEX10A)
			if (P_UsePuzzleItem(player, type))
			{
				return true;
			}
			printf("gee, I thought it would work.\n");
#else
	...
	switch(special)
	{
		...
		case 74: // Teleport_NewMap
			...
			{ // Only teleport when crossing the front side of a line
				...
				{
					if (&players[consoleplayer] == mo->player)
					{
						P_SetMessage(mo->player, "ACCESS DENIED -- DEMO", true);
						S_StartSound(mo, SFX_CHAT);
					}
				}
#endif
			}
...
#if (APPVER_HEXENREV < AV_HR_HEX10A) // From Heretic, but without setting episode var
