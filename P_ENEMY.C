...
// Macros
#if (APPVER_HEXENREV < AV_HR_HEX10A) // Heretic's boss spots definitions
...
#if (APPVER_HEXENREV < AV_HR_HEX10A) // From Heretic again
//----------------------------------------------------------------------------
//
// PROC P_InitMonsters
//
...
#endif // (APPVER_HEXENREV >= AV_HR_HEX10A) || (!defined APPVER_DEMO)
...
#else
	mo = P_SpawnMissile(actor, actor->target, MT_DEMONFX1);
#endif
	if (mo)
	{
		mo->z += 30*FRACUNIT;
...
#if (APPVER_HEXENREV < AV_HR_HEXDEMOA)
	S_StartSound(actor, SFX_SORCERER_SPELLCAST);
#else
	S_StartSound(NULL, SFX_SORCERER_SPELLCAST);
#endif
...
{
#if (APPVER_HEXENREV < AV_HR_HEXDEMOA)
	S_StartSound(actor, SFX_SORCERER_BALLPOP);
#else
...
#if (APPVER_HEXENREV < AV_HR_HEXDEMOA) // Similar to A_Chase code
//
// make active sound
//
	if(actor->info->activesound && P_Random() < 3)
...
{
#if (APPVER_HEXENREV < AV_HR_HEXDEMOA) // Earlier revision of A_FSwordAttack2
	P_SPMAngleXYZ(actor,actor->x,actor->y,actor->z-10*FRACUNIT,MT_FSWORD_MISSILE,actor->angle+ANG45/4);
	P_SPMAngleXYZ(actor,actor->x,actor->y,actor->z-5*FRACUNIT ,MT_FSWORD_MISSILE,actor->angle+ANG45/8);
	P_SPMAngleXYZ(actor,actor->x,actor->y,actor->z            ,MT_FSWORD_MISSILE,actor->angle);
	P_SPMAngleXYZ(actor,actor->x,actor->y,actor->z+5*FRACUNIT ,MT_FSWORD_MISSILE,actor->angle-ANG45/8);
	P_SPMAngleXYZ(actor,actor->x,actor->y,actor->z+10*FRACUNIT,MT_FSWORD_MISSILE,actor->angle-ANG45/4);
...
{
#if (APPVER_HEXENREV < AV_HR_HEXDEMOA) // Earlier revision of A_CHolyAttack3
	if(actor->target)
	{
