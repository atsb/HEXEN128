...
#if (APPVER_HEXENREV < AV_HR_HEX10A) // VERSIONS RESTORATION: Based on Heretic
static char cheats[] = { 'r','e','v','e','a','l',
                         'k','i','l','l','s'};
#else
...
boolean AM_Responder (event_t *ev)
{
	...
	else if (ev->type == ev_keydown)
	{
...
#if (APPVER_HEXENREV < AV_HR_HEX10A) // VERSIONS RESTORATION: From Heretic
		if(cheats[cheatcount]==ev->data1 && !netgame)
			cheatcount++;
		else
			cheatcount=0;
		if(cheatcount == 6)
		...
		if(cheats[6+ShowKillsCount] == ev->data1 && netgame && deathmatch)
#else
