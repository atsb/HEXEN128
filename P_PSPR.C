...
#if (APPVER_HEXENREV < AV_HR_HEX10A) // VERSIONS RESTORATION: A small hack
#define AdjustPlayerAngle(pmo) \
	(pmo)->angle = R_PointToAngle2((pmo)->x, \
		(pmo)->y, linetarget->x, linetarget->y);
#else
//============================================================================
//
//	AdjustPlayerAngle
...
void A_LightningClip(mobj_t *actor)
{
	...
	if(target)
	{
		...
		else
		{
			...
			actor->momy = 0;
#if (APPVER_HEXENREV < AV_HR_HEX10A)
			P_ThrustMobj(actor, actor->angle, actor->info->speed>>2);
#else
...
#if (APPVER_HEXENREV < AV_HR_HEX10A) // VERSIONS RESTORATION: A small hack
#define MStaffSpawn(pmo, angle) P_SPMAngle(pmo, MT_MSTAFF_FX2, angle)
#else
//============================================================================
//
// MStaffSpawn
...
#define pmo player->mo
...
#if (APPVER_HEXENREV < AV_HR_HEX10A)
	damage = 32+(P_Random()&15)+(P_Random()&7);
#else
	...
	{
		damage <<= 1;
#if (APPVER_HEXENREV < AV_HR_HEXDEMOA) // Later moved down, with modification
		player->mana[MANA_1] -=
			WeaponManaUse[player->class][player->readyweapon];
		if(player->mana[MANA_1] <= 0)
		{
