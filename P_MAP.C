...
#if (APPVER_HEXENREV < AV_HR_HEX10A) // Uncomment for versions preceding 1.0
	if(thing->flags2&MF2_BOSS)
...
#if (APPVER_HEXENREV < AV_HR_HEX10A)
		damage = (bombdamage*(bombdistance-dist)/bombdistance);
#else
...
#if (APPVER_HEXENREV < AV_HR_HEX10A) // From Heretic, adds redundant condition + push/pop of edi
		if (!thing->flags && false)
#else
		if ((!(thing->flags&MF_NOBLOOD)) &&
			(!(thing->flags2&MF2_INVULNERABLE)))
#endif
