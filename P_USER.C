...
void ResetBlasted(mobj_t *mo)
{
	...
	mo->flags2 &= ~(MF2_BLASTED|MF2_SLIDE);
#else
	mo->flags2 &= ~MF2_BLASTED;
#if (APPVER_HEXENREV < AV_HR_HEX10A)
	if (!(mo->flags & MF_SOLID) || !(mo->flags & MF_SHOOTABLE) ||
	    !(mo->flags2 & MF2_PUSHABLE) || !(mo->flags2 & MF2_SLIDE))
#else
...
void P_BlastMobj(mobj_t *source, mobj_t *victim, fixed_t strength)
{
	...
	else		// full strength blast from artifact
	{
		...
		{
			if (victim->type == MT_SORCBALL1 ||
			    victim->type == MT_SORCBALL2 ||
			    victim->type == MT_SORCBALL3)
			{
				return;
			}
		}
#elif (APPVER_HEXENREV >= AV_HR_HEX10A)
		if (victim->flags&MF_MISSILE)
		{
			switch(victim->type)
			{
...
void P_BlastRadius(player_t *player)
{
	...
	for(think = thinkercap.next; think != &thinkercap; think = think->next)
	{
		...
		mo = (mobj_t *)think;
#if (APPVER_HEXENREV < AV_HR_HEXDEMOA)
		if((mo == pmo) || (mo->health <= 0) || (mo->flags2&MF2_BOSS))
#else
		...
		if(!(mo->flags&MF_COUNTKILL) &&
		  !(mo->player) &&
		  !(mo->flags&MF_MISSILE) &&
		  (mo->type != MT_POISONCLOUD ? 1 : 0))
		{	// Must be monster, player, or missile
			continue;
		}
#elif (APPVER_HEXENREV < AV_HR_HEX10A)
		if (mo->type != MT_POISONCLOUD)
		{
			if ((mo->flags&MF_SOLID) &&
			   (mo->flags&MF_CORPSE) &&
			   (mo->flags2&MF2_PUSHABLE) &&
			   (mo->flags2&MF2_SLIDE))
			{
...
#else // APPVER_HEXENREV >= AV_HR_HEX10A
...
boolean P_HealRadius(player_t *player)
{
	...
	for(think = thinkercap.next; think != &thinkercap; think = think->next)
	{
		...
		if (mo->health < -99) continue;
		if (mo->flags2&MF2_BOSS) continue;
#else
		...
		if (mo->player)
		{
			if (mo == pmo)
			{
				P_GiveBody(mo->player, 100);
			}
			if (mo->health <= 0) continue;
			amount = 50 + (P_Random()%50);
			effective=true;
			P_GiveBody(mo->player, amount);
		}
		else if (mo->flags&MF_CORPSE)
		{
			...
			amount = 50 + (P_Random()%50);
			effective=true;
			if (mo->health + amount <= 0) continue;
			type = mo->type;
			x = mo->x;
			y = mo->y;
			z = mo->z;
			A_SmokePuffEntry(mo);
			cmo = P_SpawnMobj(x, y, z, type);
			if (cmo)
			{
				if (type == MT_PIG)
				{
					cmo->special1 = 40*35;
					cmo->special2 = mo->special2;
				}
				cmo->health = mo->health + amount;
				if (cmo->health > cmo->info->spawnhealth)
				{
					cmo->health = cmo->info->spawnhealth;
				}
				if (P_TestMobjLocation(cmo) == false)
				{
					P_RemoveMobj(cmo);
				}
			}
	...
	switch(arti)
	{
		...
		case arti_boostarmor:
#if (APPVER_HEXENREV < AV_HR_HEX10A)
			count = AutoArmorSave[player->class] >> FRACBITS;

			for(i = 0; i < NUMARMOR; i++)
			{
				count += player->armorpoints[i] >> FRACBITS;
			}
			count /= 5;
			if (count >= ArmorMax[player->class])
			{
				return false;
			}
			for(i = 0; i < NUMARMOR; i++)
			{
				...
				{
					count++;
					player->armorpoints[i] += 5*FRACUNIT;
				}
			}
#else // APPVER_HEXENREV >= AV_HR_HEX10A
			count = 0;
