...
#if (APPVER_HEXENREV < AV_HR_HEX10A) // Including dmx.h may impact this
#pragma on (check_stack)
#endif

// Macros

#if (APPVER_HEXENREV >= AV_HR_HEX10A) // Moved from I_SOUND.C
...
#if (APPVER_HEXENREV >= AV_HR_HEX10A) // Moved back from I_SOUND.C
...
#if (APPVER_HEXENREV >= AV_HR_HEX10A) // Moved (partially back) from I_SOUND.C
...
#endif // APPVER_HEXENREV >= AV_HR_HEX10A, moved (partially back) from I_SOUND.C
...
#else // Possibly restored after moving code back from I_SOUND.C
int             ticcount;
#endif
...
#if (APPVER_HEXENREV < AV_HR_HEX10A) // VERSIONS RESTORATION: Later moved down
...
*/
	...
	printf("HEXEN: program exited normally.\n");
...
#if (APPVER_HEXENREV < AV_HR_HEX10A)
	printf("Using external driver: %d\n", i_Vector);
#endif
...
#if (APPVER_HEXENREV >= AV_HR_HEX10A) // VERSIONS RESTORATION: Moved from above
void I_StartupReadKeys(void)
{
	int k;
