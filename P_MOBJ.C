...
void P_FloorBounceMissile(mobj_t *mo)
{
	...
	if(mo->info->seesound)
	{
#if (APPVER_HEXENREV >= AV_HR_HEX10A) && (!defined APPVER_DEMO) // // VERSIONS RESTORATION: Duplicate function call
		switch(mo->type)
		{
			case MT_SORCBALL1:
			case MT_SORCBALL2:
...
void P_XYMovement(mobj_t *mo)
{
	...
	fixed_t xmove, ymove;
	int special;
#if (APPVER_HEXENREV >= AV_HR_HEX10A) // VERSIONS RESTORATION: Used for controlling code size
	angle_t angle;
#endif
	...
	do
	{
		...
		{ // Blocked move
			...
			{ 
#if (APPVER_HEXENREV < AV_HR_HEX10A) // VERSIONS RESTORATION: Controlling code size again
				angle_t angle;
#endif
...
#if (APPVER_HEXENREV < AV_HR_HEX10A)
	if(p->class == PCLASS_FIGHTER && mthing->type != 3)
	{
		if(mthing->type == 0)
		{
...
void P_SpawnMapThing(mapthing_t *mthing)
{
	...
	{ // Single player
#if (APPVER_HEXENREV < AV_HR_HEXDEMOA)
		spawnMask = classFlags[PlayerClass[0]];
#else
...
#if (APPVER_HEXENREV < AV_HR_HEXDEMOA)
	if((mthing->options&spawnMask) != spawnMask)
	{
		return;
	}
#endif
