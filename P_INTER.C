...
#else // APPVER_HEXENREV < AV_HR_HEX10A: A bit of a hack
#define TryPickupWeapon(player, weaponClass, weaponType, weapon, message) \
	if (P_GiveWeapon(player, weaponClass, weaponType)) \
	{ \
		P_SetMessage(player, message, false); \
...
#if (APPVER_HEXENREV < AV_HR_HEX10A) // Uncomment for older versions
...
#if (APPVER_HEXENREV < AV_HR_HEX10A) // Uncomment for older versions again
...
#if (APPVER_HEXENREV < AV_HR_HEX10A) // A bit of a hack
#define TryPickupWeaponPiece(player, matchClass, pieceValue, pieceMobj) \
	if (P_GiveWeaponPiece(player, matchClass, pieceValue)) \
	{ \
		sound = SFX_PICKUP_PIECE; \
		break; \
	}
#else
static void TryPickupWeaponPiece(player_t *player, pclass_t matchClass,
...
#if (APPVER_HEXENREV < AV_HR_HEX10A) // From Heretic, later relocated above
//----------------------------------------------------------------------------
//
// PROC P_HideSpecialThing
//
...
void P_TouchSpecialThing(mobj_t *special, mobj_t *toucher)
{
	...
	switch(special->sprite)
	{
		...
		case SPR_MAN1:
#if (APPVER_HEXENREV < AV_HR_HEX10A)
			if(!P_GiveMana(player, MANA_1, 10))
#else
		...
		case SPR_MAN2: 
#if (APPVER_HEXENREV < AV_HR_HEX10A)
			if(!P_GiveMana(player, MANA_2, 10))
#else
...
void P_KillMobj(mobj_t *source, mobj_t *target)
{
	...
	{ // Player death
		...
		{ // Player ice death
#if (APPVER_HEXENREV < AV_HR_HEX11A)
			target->flags &= ~(3<<MF_TRANSSHIFT); //no translation
#else
...
#else // APPVER_DEMO
		if(target->type == MT_TREEDESTRUCTIBLE)
#endif
		{
...
#if (APPVER_HEXENREV < AV_HR_HEX10A)
		else if ((target->flags&MF_SOLID) &&
		         (target->flags&MF_CORPSE) &&
		         (target->flags&MF_SHOOTABLE) &&
		         (target->flags2&MF2_PUSHABLE))
#else
