...
#if (APPVER_HEXENREV < AV_HR_HEX10A)
fixed_t MaxPlayerMove[NUMCLASSES] = { 0x3C, 0x32, 0x2D, 0x40 };
#else
...
#if (APPVER_HEXENREV < AV_HR_HEXDEMOA) // Later moved up
	memset(player->powers, 0, sizeof(player->powers));
#endif
...
void G_Completed(int map, int position)
{
 ...
 P_SetMessage(&players[consoleplayer], "ACCESS DENIED -- BETA", true);
 S_StartSound(players[consoleplayer].mo, SFX_CHAT);
#endif
}
...
#if (APPVER_HEXENREV >= AV_HR_HEXDEMOA) // As in Heretic
