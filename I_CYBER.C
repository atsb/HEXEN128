...
#include <stdlib.h>
#include <string.h>
#if (APPVER_HEXENREV >= AV_HR_HEX10A) // Wrong ST_Message signature is otherwise assumed
...
void I_StartupCyberMan(void)
{
...
#if (APPVER_HEXENREV < AV_HR_HEX10A)
	  ST_Message("CyberMan: Wrong mouse driver - no SWIFT support (AX=%04x).\n",
   ...
   else if (pbuf->deviceType != DEVTYPE_CYBERMAN)
   {
	  ...
	  {
#if (APPVER_HEXENREV < AV_HR_HEX10A)
		 ST_Message("CyberMan: no SWIFT device connected.\n");
#else
	  ...
	  {
#if (APPVER_HEXENREV < AV_HR_HEX10A)
		 ST_Message("CyberMan: SWIFT device is not a CyberMan! (type=%d)\n",
				pbuf->deviceType);
#else
   ...
   {
#if (APPVER_HEXENREV < AV_HR_HEX10A)
	  ST_Message("CyberMan: CyberMan %d.%02d connected.\n",
